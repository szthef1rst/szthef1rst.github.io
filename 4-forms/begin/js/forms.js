...
function sendData() {
  ...
  // radio buttons
  let radio = document.getElementsByName("experience");
  for (var i = 0, length = radio.length; i < length; i++) {
    if (radio[i].checked) {
      urlEncodedDataPairs.push(
        encodeURIComponent("experience") + "=" + encodeURIComponent(radio[i].value)
      );
    }
  }

  // dropdown menu
  var dropdown = form.querySelector("[name='experience']");
  urlEncodedDataPairs.push(
